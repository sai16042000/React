...
app.post("/login",(req,res)=>{
     ashokIT.connect("mongodb+srv://admin:admin@cluster0.4szyv.mongodb.net/R-08?retryWrites=true&w=majority",
                            (err,connection)=>{
            if(err) throw err;
            else{
                const db = connection.db("R-08");
                db.collection("login-details").find({"username":req.body.username,"password":req.body.password})
                    .toArray((err,array)=>{
                        if(err) throw err;
                        else{
                            if(array.length>0){
